#' @importFrom dplyr ungroup mutate case_when filter arrange desc select across
#' @importFrom dplyr all_of group_by summarize summarise bind_rows n nest_by
#' @importFrom dplyr inner_join
#' @importFrom rlang sym syms .data :=
#' @importFrom tidyr complete nesting pivot_longer unnest nest
#' @importFrom tibble add_column
#' @importFrom ggplot2 ggplot aes geom_path geom_bar geom_line geom_abline
#' @importFrom ggplot2 geom_point xlim ylim facet_wrap element_text theme
#' @importFrom ggplot2 coord_cartesian position_dodge labs
  ...
  R6::R6Class(
    ...
    public = list(
      ...
      initialize = function(
        ...
        self$smc <- .summarise_missing_contrasts(
          self$.data,
          hierarchy = hierarchy,
          contrast = contrast,
          what = summarizeNA
      ...
      plot_ROC = function(xlim = 0.5) {
        ...
        vissum <- .plot_ROC(confusion, fpr_lim = xlim, contrast = self$contrast)
        return(vissum)
      },
      #' @description
      #' AUC summaries
