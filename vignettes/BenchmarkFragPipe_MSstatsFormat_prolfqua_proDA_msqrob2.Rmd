...
evalAll <- requireNamespace("proDA", quietly = TRUE) && requireNamespace("MSstats", quietly = TRUE)
```
...
```{r prepareMSstatsData, eval = evalAll}
...
```{r setUPBenchmarkMSstats, eval = evalAll}
library(prolfqua)
ttd <- ionstar_bench_preprocess(bb, idcol = "Protein")
benchmark_msstats <- make_benchmark(
    ttd$data,
    contrast = "Label",
    toscale = c("pvalue"),
    fcestimate = "log2FC",
