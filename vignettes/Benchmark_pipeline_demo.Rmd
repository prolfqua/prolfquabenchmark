...
lfqdata$set_data(lfqdata$data_long() |> dplyr::filter(!grepl("^REV__|^CON__", protein_Id)))
...
lfqdataNormalized <- tr$log2()$robscale_subset(lfqsubset = subset_h, preserve_mean = FALSE)$lfq

# Median polish aggregation
lfqAggMedpol <- lfqdataNormalized$get_Aggregator()
lfqAggMedpol$aggregate()
...
```{r fitLM}
lmmodel <- paste0(protLFQ$response(), " ~ dilution.")
modelFunction <- prolfqua::strategy_lm(lmmodel, model_name = "Model")
modLinearProt <- prolfqua::build_model(protLFQ, modelFunction)
