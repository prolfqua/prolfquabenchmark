...
exampleDat <- lfqtrans$data |> dplyr::mutate(CorT = dplyr::case_when(dilution. == "a" ~ "C", TRUE ~ "T"))
...
Ecolifasta <- seqinr::read.fasta(Ecolifasta, as.string = TRUE, seqtype = "AA")
Humanfasta <- seqinr::read.fasta(Humanfasta, as.string = TRUE, seqtype = "AA")
...
saveRDS(benchmark_SaintExpres, file = file.path("../inst/Benchresults/",format( Sys.Date(), "%Y%m%d"),"benchmark_medpolish_SaintExpress.RDS"))
