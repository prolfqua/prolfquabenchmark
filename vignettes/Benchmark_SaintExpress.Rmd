...
- Secondly, the score with the highest performance (largest AUC) is the log2 fold change. Our expectations would be however, that SaintScore or BFDR should perform better, when assuming that they correctly model the observed variances.
- Third, the performance of _SaintExpress_  is significantly worse when compared with other algorithms, e.g. _limma_, _proDA_ or _PRORA_ (see other benchmark vignettes).
...
- the SaintScore - Saint Probability, vaguely - probability that the protein is a true interactor.
...
_SaintExpress_ does not report the PEP. But assuming that $PEP = 1 - SaintScore$ we can compute the BFDR:
