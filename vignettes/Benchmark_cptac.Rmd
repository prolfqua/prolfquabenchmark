...
The r-package `r BiocStyle::Biocpkg("msdata")` msdata provides a subset of the data of the 6th study of the Clinical Proteomic Technology Assessment for Cancer (CPTAC).
In this experiment, the Sigma Universal Protein Standard mixture 1 (UPS1) containing 48 different human proteins was spiked in a protein background of 60 ng/$\muL$ Saccharomyces cerevisiae strain BY4741. Two different spike-in concentrations were used, 6A ($0.25$ fmol UPS1 proteins/$\muL$) and 6B ($0.74$ fmol UPS1 proteins/$\muL$). Three replicates are available for each concentration.
...
tmp <- c(msqrob_vs_prolfqua = pROC::roc.test(ddmsqrob,ddprolfqua)$p.value,
         msqrob_vs_proda = pROC::roc.test(ddmsqrob,ddproda)$p.value,
         prolfqua_vs_proda = pROC::roc.test(ddprolfqua,ddproda)$p.value)

```

We observe that for this benchmark data, the three packages perform similarly
with the following results of the Bootstrap test for two ROC curves:

```{r}
knitr::kable(data.frame(pROC_test = names(tmp), p.value = tmp))
```
