...
```{r pkgNotice, echo=FALSE, results='asis'}
if (!evalAll) {
  cat("> **Note:** This vignette requires the `proDA` package, which is not currently installed.",
      "All code chunks are skipped. Install it and re-render to see results.\n")
}
```
...
config$file_name <- "raw.file"
config$set_response("mq.protein.lfq.intensity")
...
lfqhum <- lfqPROT$get_subset(dplyr::filter(lfqPROT$data_long(), grepl("HUMAN", proteinID)))
...
wide <- lfqtrans$data_wide(as.matrix = TRUE)
...
mqdata2$config <- create_mq_peptide_config()

annotation <- readxl::read_xlsx(inputAnnotation)
res2 <- dplyr::inner_join(mqdata2$data, annotation, by = "raw.file")

mqdata2$config$factors[["dilution."]] <- "sample"
mqdata2$config$factors[["run_Id"]] <- "run_ID"
mqdata2$config$factor_depth <- 1
mqdata2$data <- prolfqua::setup_analysis(res2, mqdata2$config)

lfqdata2 <- prolfqua::LFQData$new(mqdata2$data, mqdata2$config)
lfqdata2$set_data(lfqdata2$data_long() |> dplyr::filter(!grepl("^REV__|^CON__", protein_Id)))
...
subset_h2$set_data(subset_h2$data_long() |> dplyr::filter(grepl("HUMAN", protein_Id)))
...
subset_h3$set_data(subset_h3$data_long() |> dplyr::filter(grepl("HUMAN", protein_Id)))
...
subset_h4$set_data(subset_h4$data_long() |> dplyr::filter(grepl("HUMAN", protein_Id)))
...
config5 <- create_mq_peptide_config()
config5$factors["group."] <- "group"
