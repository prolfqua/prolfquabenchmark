...
lfqNormSubset$config$table$get_work_intensity()
...
lmmodel <- "~ dilution."
lmmodel <- paste0(protLFQ$config$table$get_work_intensity() , lmmodel)
...
lmmodel <- paste0(lfqNormSubset$config$table$get_work_intensity() , lmmodel)
lfqNormSubset$config$table$hierarchyDepth <- 1
...
prolfqua::table_facade(tt, caption = "All possible Contrasts given 5 E. coli dilutions of the Ionstar Dataset", digits=1)
...
```{r mergingContrasts}
all <- prolfqua::merge_contrasts_results(prefer = contrProtModerated, add = contrImp)
