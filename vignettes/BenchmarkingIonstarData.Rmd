...
ttd <- ionstar_bench_preprocess(contrImp$get_contrasts())

benchmark_missing <- make_benchmark(
    ttd$data,
...
ttd <- ionstar_bench_preprocess(contrProt$get_contrasts())

benchmark_Prot <- make_benchmark(
    ttd$data,
...
ttd <- ionstar_bench_preprocess(contrProtModerated$get_contrasts())
benchmark_ProtModerated <- make_benchmark(
    ttd$data,
...
### Adding DEqMS Moderation

DEqMS applies count-dependent variance shrinkage: proteins quantified from many peptides get less shrinkage, proteins from few peptides get more.

```{r buildCountDF}
count_df <- lfqNormSubset$data |>
  dplyr::select(protein_Id, peptide_Id) |>
  dplyr::distinct() |>
  dplyr::count(protein_Id, name = "nr_peptides")
```

```{r contrastModel1DEqMS}
contrProtDEqMS <- prolfqua::ContrastsModeratedDEqMS$new(
  contrProt,
  count_df = count_df,
  count_column = "nr_peptides"
)
contrProtDEqMS$get_Plotter()$volcano()$FDR
```

```{r benchmarkModel1DEqMS}
allContrasts$ProtDEqMS <- contrProtDEqMS$get_contrasts()
ttd <- ionstar_bench_preprocess(contrProtDEqMS$get_contrasts())
benchmark_ProtDEqMS <- make_benchmark(
    ttd$data,
    model_description = "med. polish and lm DEqMS moderated",
    model_name = "prolfqua_lm_DEqMS")

prolfqua::table_facade(
  benchmark_ProtDEqMS$smc$summary,
  caption = "Nr of proteins with Nr of not estimated contrasts.",
  digits = 1)

benchmark_ProtDEqMS$plot_FDRvsFDP()
allBenchmarks$benchmark_ProtDEqMS <- benchmark_ProtDEqMS
```
...
ttd <- ionstar_bench_preprocess(contrProtMixed$get_contrasts())
benchmark_mixed <- make_benchmark(
    ttd$data,
...
ttd <- ionstar_bench_preprocess(contrProtMixedModerated$get_contrasts())

benchmark_mixedModerated <- make_benchmark(
    ttd$data,
...
ttd <- ionstar_bench_preprocess(cr)
benchmark_ropeca <- make_benchmark(
    ttd$data,
...
## Contrasts from limma model

Limma uses a matrix-based linear model with empirical Bayes variance moderation via `lmFit` + `eBayes`.
This is an alternative to the per-protein `lm` + `squeezeVarRob` pipeline.

```{r buildModelLimma}
limma_formula <- paste0(protLFQ$config$table$get_response(), " ~ dilution.")
limma_strategy <- prolfqua::strategy_limma(limma_formula)
modLimma <- prolfqua::build_model_limma(protLFQ, limma_strategy)
modLimma$anova_histogram()$plot
```

```{r contrastLimma}
contrLimma <- prolfqua::ContrastsLimma$new(modLimma, relevantContrasts)
contrLimma$get_Plotter()$volcano()$FDR
```

```{r benchmarkLimma}
allContrasts$Limma <- contrLimma$get_contrasts()
ttd <- ionstar_bench_preprocess(contrLimma$get_contrasts())
benchmark_Limma <- make_benchmark(
    ttd$data,
    model_description = "med. polish and limma",
    model_name = "prolfqua_limma")

prolfqua::table_facade(
  benchmark_Limma$smc$summary,
  caption = "Nr of proteins with Nr of not estimated contrasts.",
  digits = 1)

benchmark_Limma$plot_FDRvsFDP()
allBenchmarks$benchmark_Limma <- benchmark_Limma
```

## Robust Linear Model (RLM)

Robust linear model fitted to protein abundances using `strategy_rlm`, with variance moderation via `ContrastsModerated`.

```{r contrastRLM}
fa_rlm <- prolfqua::ContrastsRLMFacade$new(protLFQ, "~ dilution.", relevantContrasts)
fa_rlm$get_Plotter()$volcano()$FDR
```

```{r benchmarkRLM}
allContrasts$RLM <- fa_rlm$get_contrasts()
ttd <- ionstar_bench_preprocess(fa_rlm$get_contrasts())
benchmark_rlm <- make_benchmark(
    ttd$data,
    model_description = "med. polish and rlm moderated",
    model_name = "prolfqua_rlm")

prolfqua::table_facade(
  benchmark_rlm$smc$summary,
  caption = "Nr of proteins with Nr of not estimated contrasts.",
  digits = 1)

benchmark_rlm$plot_ROC(xlim = 0.2)
benchmark_rlm$plot_FDRvsFDP()
allBenchmarks$benchmark_rlm <- benchmark_rlm
```
...
ttd <- ionstar_bench_preprocess(merged$get_contrasts())
...
### Merged with DEqMS moderation

```{r mergingContrastsDEqMS}
merged_deqms <- prolfqua::ContrastsModeratedDEqMS$new(
  all$merged,
  count_df = count_df,
  count_column = "nr_peptides"
)
ttd <- ionstar_bench_preprocess(merged_deqms$get_contrasts())
benchmark_merged_deqms <- make_benchmark(
    ttd$data,
    model_description = "merge of prot DEqMS moderated and imputed",
    model_name = "prolfqua_merged_DEqMS")

benchmark_merged_deqms$plot_FDRvsFDP()
allBenchmarks$benchmark_merged_deqms <- benchmark_merged_deqms
```

### Merged with limma

```{r mergingContrastsLimma}
all_limma <- prolfqua::merge_contrasts_results(prefer = contrLimma, add = contrImp)
merged_limma <- prolfqua::ContrastsModerated$new(all_limma$merged)
ttd <- ionstar_bench_preprocess(merged_limma$get_contrasts())
benchmark_merged_limma <- make_benchmark(
    ttd$data,
    model_description = "merge of limma and imputed",
    model_name = "prolfqua_limma_merged")

benchmark_merged_limma$plot_FDRvsFDP()
allBenchmarks$benchmark_merged_limma <- benchmark_merged_limma
```
...
ttd <- ionstar_bench_preprocess(same$get_contrasts())
benchmark_same <- make_benchmark(
    ttd$data,
...
```{r saveBenchmarks, include=FALSE}
bdir <- file.path("../inst/Benchresults/",format( Sys.Date(), "%Y%m%d"))
if (!dir.exists(bdir)) {dir.create(bdir)}

if (SAVE) {
    if (SUBSETNORM) {
        saveRDS(allBenchmarks, file = file.path("../inst/Benchresults/",format( Sys.Date(), "%Y%m%d"),"allBenchmarks.RDS"))
        saveRDS(allContrasts, file = file.path("../inst/Benchresults/",format( Sys.Date(), "%Y%m%d"),"allContrasts.RDS"))
    } else{
        saveRDS(allContrasts, file = file.path("../inst/Benchresults/",format( Sys.Date(), "%Y%m%d"),"allContrastsNORMAL.RDS"))
        saveRDS(allBenchmarks, file = file.path("../inst/Benchresults/",format( Sys.Date(), "%Y%m%d"),"allBenchmarksNORMAL.RDS"))
    }
...
|--------------------------|:---------------:|----------|------------------|-------------|
| Protein abundance        |       lm        | o        | o                |             |
| Protein abundance        |       lm        | o        | o (DEqMS)        |             |
| Protein abundance        |      limma      | o        | o (eBayes)       |             |
| Protein abundance Imputed| pooled variance | o        | o                |             |
| Peptide abundance        |      lmer       | o        | o                |             |
| Peptide abundance        |       lm        |          |                  | o           |

```{r comparepAUC10, fig.cap="Partial area under the ROC curve at 10% FPR."}
...
```{r comparepAUC20, fig.caption="Partial area under the ROC curve at 20% FPR."}
...
```{r compareNrEstimatedContrasts, fig.cap="NR of estimated contrasts", fig.width=5, fig.height=5}
