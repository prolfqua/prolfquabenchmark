...
information. You do it using the `AnalysisConfiguration` class.


The `AnalysisConfiguration` has the following fields that need to be populated:
- file_name
- hierarchy
- factors
- work_intensity
...
The `file_name` is the column with the raw file names, however for labelled TMT experiments, it can be used to hold the name of the TMT channel.
...
config <- create_mq_peptide_config()
...
config$factors[["F1."]] = "F1"
config$factors[["F2."]] = "F2"
...
protInt$set_data(protInt$data_long() |> dplyr::filter(protein_Id %in% protIntSig$protein_Id))
protInt$hierarchy_counts()
mean(grepl("ECOLI", protInt$data_long()$protein_Id))
...
```{r}
lfqTrans$set_config_value("factor_depth", 2)
