...
b1 <- allBenchmarks$benchmark_imputation$pAUC
b1$get_confusion_benchmark

allBenchmarks[[1]]$pAUC

res <- purrr::map_df(allBenchmarks, function(x){x$pAUC()})
res <- res |> dplyr::mutate(whatfix = case_when(what == "scaled.beta.based.significance" ~ "scaled.p.value", TRUE ~ what))
...
p1 <- ggplot2::ggplot(resAllB, ggplot2::aes(x = Name, y = pAUC_10)) +
  ...
  ggplot2::geom_hline(ggplot2::aes(yintercept = meanpAUC_10), color = "red") + xlab("")
...
p4 <- ddb |> ggplot(aes(y = FDP_,  x  = scorecol )) + 
