...
abstract <- tex[
  grep("begin\\{abstract\\}", tex):(grep("end\\{abstract\\}", tex))
]
writeLines(abstract, "../../../prolfqua-achems/abstract.tex")

introduction <- tex[
  grep("hypertarget\\{introduction\\}", tex):(grep(
    "hypertarget\\{methods\\}",
    tex
  ) -
    1)
...
methods <- tex[
  grep("hypertarget\\{methods\\}", tex):(grep(
...
# create results.tex
results <- tex[
  grep("hypertarget\\{results-and-discussion\\}", tex):(grep(
...
results <- append(
  results,
  "\\TODO{codesnippets/codesnippet2.pdf}",
  after = (sB[1] - 1)
)
...
results <- append(
  results,
  "\\TODO{codesnippets/codesnippet3.pdf}",
  after = (sB[1] - 1)
)
...
conclusion <- tex[
  grep("hypertarget\\{conclusion\\}", tex):(grep(
...
abbreviations <- tex[
  grep("hypertarget\\{abbreviations\\}", tex):(grep(
...
abbreviations <- tex[
  grep("hypertarget\\{supporting-information\\}", tex):(grep(
