...
Computing the statistics at the peptide level, e.g., the $t$-statistics or $p$-value, then summarizing these statistics using their median produces the highest $AUC$ scores among all the tested models (see Figure \@ref(fig:FDRfdp) Panel A left). Furthermore, by using the Beta distribution to model the number of peptides observed, we can further improve the $pAUC$ scores (see Figure \@ref(fig:FDRfdp) Panel A center). However, the properties of Beta-based probabilities are not well understood; for instance, the p-values are not uniformly distributed under the null hypothesis. Furthermore, the $FDR$ estimates obtained when correcting for multiple testing with the Benjamini-Hochberg method are biased and overestimate the false discovery proportion (see Figure \@ref(fig:FDRfdp) Panel C). Therefore, we can not recommend this method if an unbiased estimate of $FDR$ is essential, which is frequently the case. In addition, peptides are stronger affected by missing values, reducing the number of contrasts we could estimate for the dataset using this method (see Figure \@ref(fig:FDRfdp) Panel C).

The R-package `r BiocStyle::Biocpkg("proDA")`, `r BiocStyle::Biocpkg("msqrob2")` and `r BiocStyle::Biocpkg("prolfqua")` (see Methods) model the missing data directly, while MSstats imputes the data using an accelerated failure model. Despite imputation, `r BiocStyle::Biocpkg("MSstats")` estimates group differences for less proteins and does not achieve a higher $pAUC_{10}$. Furthermore, Figure \@ref(fig:FDRfdp) Panel C shows that when using `r BiocStyle::Biocpkg("MSstats")` the proportion of false discoveries might be very high even when filtering for a low $FDR$.


We obtained difference estimates for all proteins, as shown in Figure \@ref(fig:FDRfdp) Panel A when using the probabilistic dropout model (`r BiocStyle::Biocpkg("proDA")`), the hurdle model (`r BiocStyle::Biocpkg("msqrob2")`), and when combining the results of the linear model with contrast estimates in the presence of missing data (prolfqua_merged). Furthermore, the performance of the scaled $p$-values or the $t$-statistics is comparable with that of the other methods (Figure \@ref(fig:FDRfdp) Panel B). All three models perform similarly well when benchmarked against the IonStar/MaxQuant, IonStar/FragPipe, or CPTAC dataset (see Supplement, Benchmark CPTAC). We tested if the $pAUC_{10}$ differences for the three methods were statistically significant, and none of them were at a significance level of $0.4$ (see Supplement, Testing ROC curves). Furthermore, for the IonStar/MaxQuant dataset, msqrob2 performed best, proDA performed best for the CPTAC/MaxQuant data, and prolfqua_merged for the IonStar/FragPipe dataset. Also, the FDR estimates (Figure \@ref(fig:FDRfdp) Panel C) are comparable for all three methods.
