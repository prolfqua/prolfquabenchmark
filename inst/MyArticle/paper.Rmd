...
Given the weights $c$ contrasts from model parameters obtained with `r BiocStyle::Biocpkg('proDA')`, or `r BiocStyle::Biocpkg('msqrob2')` can be determined, which will enable us to implement adapters to `proDA` or `msqrob2`.
...
To estimate $A_{LOD}$ we are using the protein abundances of those groups where the protein was observed in only a single sample (see Supplement section Estimating $A_{LOD}$). Typically there are many such cases, and hence we take the median.
