...
The probabilistic dropout analysis implemented in  `r BiocStyle::Biocpkg("proDA")`, as well as the hurdle model implemented in `r BiocStyle::Biocpkg("msqrob2")`, produce inferences comparable to those of other methods (Figure \@ref(fig:FDRfdp) Panel A). Because of the robustness of the dropout and hurdle model to missing observations we obtain difference estimates for all proteins and contrasts (Figure \@ref(fig:FDRfdp) Panel B). Moreover, the estimated differences of the probabilistic dropout model are unbiased and show high diagnostic accuracy (Figure \@ref(fig:FDRfdp) Panel A). Furthermore, the performance of the scaled $p$-values or the $t$-statistics is comparable with that of the linear model with variance moderation (`prot_med_lm_moderated`). Therefore, we are planning to integrate the `r BiocStyle::Biocpkg("proDA")` package as an additional modeling option into `r BiocStyle::Githubpkg("fgcz/prolfqua")` (see Figure \@ref(fig:ContrastUML)).
...
`r BiocStyle::Githubpkg("fgcz/prolfqua")` is an feature-rich, object oriented R package to analyze quantitative mass spectrometric data with simple or complex experimental designs. While other R packages for differential expression analysis of proteins typically only implement one modeling approach, `r BiocStyle::Githubpkg("fgcz/prolfqua")` supports various types of models (see Figure 2 and Table 3). The contrast specification is explicit and consistent for all models. The modular design of `r BiocStyle::Githubpkg("fgcz/prolfqua")` allows for adding new features, e.g., generalized linear models to model the presence or absence information of a protein, or robust linear models, in the future. Furthermore, the developed framework is flexible enough to integrate other modeling methods, e.g., the probabilistic dropout model [@bioxrvproDA2020] or accurate variance estimation [@zhu2020deqms]. Hence, `r BiocStyle::Githubpkg("fgcz/prolfqua")` enables you to call various methods and quickly select the best differential expression analysis algorithm for your problem. It also can generate conclusive reports and can be used to benchmark MS software and statistical methods. Furthermore, with minimal adaptations, this R package can analyze different quantitative proteomics data (e.g., labeling-based TMT-, PRM- DIA-data).
