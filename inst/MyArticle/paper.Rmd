...
```{r exampleContrasts, fig.cap="(ref:exampleContrasts)", echo=TRUE, out.width = '90%', fig.width = 8, fig.height = 6}
contrasts <- c(
...
This integration enables our users to select the input and basic settings in a graphical user interface. As an output, the user receives a report including the R markdown file and all necessary input files to reprocess the analysis using R on their devices. In this way, we also support the reproducible science efforts of the community.
...
Finally, we have to specify which columns contain the explanatory variables of the analysis. The AnalysisTableAnnotation has the field factors, a list with as many entries as explanatory variables. Here we have only one explanatory variable, the dilution, specified in the column 'sample.'
...
Here we compare the benchmark results for the `r BiocStyle::Biocpkg("proDA")` model and the linear model with variance moderation, but using only the protein where inference could be performed for both models. We see that the fold change estimates obtained with the `r BiocStyle::Biocpkg("proDA")` package show better diagnostic accuracy. Figure \@ref(fig:proDAAllBenchmarks), compares the fold change estimates obtained using both methods. However, the $t-statistics$ or $p-values$ show slightly worse diagnostic accuracy. 
...
```{r proDAAllBenchmarks, fig.cap="Comparison of fold change estimates produced by proDA and using the missing data modelling method implemented in prolfqua."}
