...
The linear model explains the observed protein abundances using the explanatory variables `condition_` and `batch_` and the interaction term  `condition_:batch_`. The treatment difference among the groups `Ethanol` and `Glucose` defined by the factor `condition`, and similarly among the groups defined by `batch_`, can be specified. Furthermore, the treatment differences among `Ethanol` and `Glucose` within the batches `p2370` and `p2691` can be specified. Finally, we can examine if the treatment differences in both batches are the same or are different. The code to compute the array of weights $c$ used to multiply the model coefficient $\beta$, to obtain the effect size estimates, and the variance of the estimate, is shown below:
...
Given the weights $c$ contrasts from model parameter estimates obtained with `r BiocStyle::Biocpkg('proDA')`, or `r BiocStyle::Biocpkg('msqrob2')` can be determined, by this enabling us to call these packages using the same application programming interface (API).
