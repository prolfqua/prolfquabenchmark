...
The linear model below explain the observed protein abundances using the explanatory variables `condition_` and `batch_` and the interaction term  `condition_:batch_`: 
...
The treatment difference among the groups `Ethanol` and `Glucose` defined by the factor `condition`, and similarly among the groups defined by `batch_`, can be specified.
...
Furthermore, the treatment differences among `Ethanol` and `Glucose` within the batches `p2370` and `p2691` can be specified.
...
Finally, we can examine if the treatment differences in both batches are the same or are different.

```{r contrast3, eval=TRUE, echo=TRUE}
d3 <- c("difference_of_difference" = "difference_within_p2370 - difference_within_p2691")
```

The code to compute the array of weights $c$ used to multiply the model coefficient $\beta$, to obtain the effect size estimates, and the variance of the estimate, is shown below:
...
histpval <- pl$histogram()$p.value + ggplot2::labs(tag = "A")               maplot <- pl$ma_plot(rank = TRUE) + ggplot2::labs(tag = "C")
