...
We obtained difference estimates for all proteins, as shown in Figure \@ref(fig:FDRfdp) Panel A when using the probabilistic dropout model (`r BiocStyle::Biocpkg("proDA")`), the hurdle model (`r BiocStyle::Biocpkg("msqrob2")`), and when combining the results of the linear model with contrast estimates in the presence of missing data (prolfqua_merged). Furthermore, the performance of the scaled $p$-values or the $t$-statistics is comparable with that of the other methods (Figure \@ref(fig:FDRfdp) Panel B). All three models perform similarly well when benchmarked against the IonStar/MaxQuant, IonStar/FragPipe, or CPTAC dataset (supplement). We tested if the $pAUC_{10}$ differences for the three methods were statistically significant, and none of them were at a significance level of $0.4$ (see supplement). Furthermore, for the IonStar/MaxQuant dataset, msqrob2 performed best, proDA performed best for the CPTAC/MaxQuant data, and prolfqua_merged for the IonStar/FragPipe dataset. Also, the FDR estimates (Figure \@ref(fig:FDRfdp) Panel C) are comparable for all three methods.
