...
The linear model below explain the observed protein abundances using the explanatory variables `condition_` and `batch_` and the interaction among them `condition_:batch_`, 

```{r, echo = TRUE}
data_Yeast2Factor <- prolfqua::prolfqua_data("data_Yeast2Factor")
mlm <- prolfqua::strategy_lm(transformedIntensity ~ condition_ + batch_ + condition_:batch_)
mm <- prolfqua::build_model(data_Yeast2Factor$data, mlm, subject = "protein_Id")
```

then the contrasts among the groups `Ethanol` and `Glucose` defined by the factor `condition`, and similarly for the difference of the groups defined by `batch_`, can be specified as:

```{r contrast1,  echo=TRUE}
d1 <- c("diff_condition" = "condition_Ethanol - condition_Glucose",
  "diff_batch" = "batch_p2370 - batch_p2691")
```

Furthermore, the difference among `Ethanol` and `Glucose` within the batches `p2370` and `p2691` can be specified as follows:

```{r contrast2, eval=TRUE, echo=TRUE}
d2 <- c("difference_within_p2370" =
    "`condition_Glucose:batch_p2370` - `condition_Ethanol:batch_p2691`",
  "difference_within_p2691" = 
   "`condition_Glucose:batch_p2691` - `condition_Ethanol:batch_p2691`")
```

Finally, we can examine if the differences among `conditions_` observed in both batches significantly differ by specifying the contrast: 

```{r contrast3, eval=TRUE, echo=TRUE}
d3 <- c("difference_of_difference" = "difference_within_p2370 - difference_within_p2691")
```

The code to compute the array of weights $c$ used to multiply the model coefficient $\beta$, to obtain the effect size  estimates, is shown below:

```{r contrast4, echo=TRUE}
Contrasts <- c(d1, d2, d3)
linfct <- prolfqua::linfct_from_model(mm$modelDF$linear_model[[1]],
                                      as_list = FALSE)
mC <- prolfqua::linfct_matrix_contrasts(linfct, Contrasts)
```

while Table \@ref(tab:contrcoefficient) shows the weight $c$ of each model parameter in each contrast.

```{r contrcoefficient}
knitr::kable(mC, caption = "Weights of each model parameter.",format = "latex")

```


This example is taken from the `r BiocStyle::Githubpkg("fgcz/prolfqua")` package vignette file.
Given the coefficients, $c$ contrasts for models obtained with `r BiocStyle::Biocpkg('proDA')`, or `r BiocStyle::Biocpkg('msqrob2')` can be determined, enabling us to integrate these packages transparently.
...
# create proflqua configuration
config <- prolfqua::create_config_MQ_peptide()
config$table$factors[['dilution.']] = "sample"
## create R6 object
lfqd <- prolfqua::LFQData$new(startdata, config, setup = TRUE) 
...
lfqd <- lfqd$get_Transformer()$log2()$robscale()$lfq
lfqd$rename_response("log_peptide_abundance")
agr <- lfqd$get_Aggregator()
lfqp <- agr$medpolish()
lfqp$rename_response("log_protein_abundance")
## plot panels A-D
pl <- lfqd$get_Plotter()
