...
Since only technical replicates are available for each dilution, essential sources of variation present in every experiment, such as the biochemical and biological, are not measured. Therefore, the IonStar dataset (see Methods) captures only the variance from the chromatography, electro-spray, and mass spectrometric measurements. Furthermore, this benchmark with a parallel group design does not allow for benchmarking models with interactions. Unfortunately,  other benchmark datasets have similar constraints [@edwards2015cptac, @o2018proteome]. At the same time, we could not analyze the benchmark dataset [@wessels2012comprehensive] with a full factorial design with an up-to-date version of MaxQuant.
Thus, while we can extrapolate some of the results obtained to more realistic datasets, we need to be careful not to over-interpret. Specifically, the observed variances will be higher in more realistic experiments, and the power will be lower for the same sample sizes. Furthermore, the proportion of missing observations in real-life datasets might also be higher.
...
The R-package `r BiocStyle::Biocpkg("proDA")`, `r BiocStyle::Biocpkg("msqrob2")` and `r BiocStyle::Biocpkg("prolfqua")` (see Methods) model the missing data directly, while MSstats imputes the data using an accelerated failure model. Despite imputation, `r BiocStyle::Biocpkg("MSstats")` estimates group differences for less proteins and does not achieve a higher $pAUC_{10}$. Furthermore, Figure \@ref(fig:figure3) Panel C shows that when using `r BiocStyle::Biocpkg("MSstats")` the proportion of false discoveries might be very high even when filtering for a low $FDR$.


We obtained difference estimates for all proteins, as shown in Figure \@ref(fig:figure3) Panel A when using the probabilistic dropout model (`r BiocStyle::Biocpkg("proDA")`), the hurdle model (`r BiocStyle::Biocpkg("msqrob2")`), and when combining the results of the linear model with contrast estimates in the presence of missing data (prolfqua_merged). Furthermore, the performance of the scaled $p$-values or the $t$-statistics is comparable with that of the other methods (Figure \@ref(fig:figure3) Panel B). All three models perform similarly well when benchmarked against the IonStar/MaxQuant, IonStar/FragPipe, or CPTAC dataset (see Supplement, Benchmark CPTAC). We tested if the $pAUC_{10}$ differences for the three methods were statistically significant, and none of them were at a significance level of $0.4$ (see Supplement, Testing ROC curves). Furthermore, for the IonStar/MaxQuant dataset, msqrob2 performed best, proDA performed best for the CPTAC/MaxQuant data, and prolfqua_merged for the IonStar/FragPipe dataset. Also, the FDR estimates (Figure \@ref(fig:figure3) Panel C) are comparable for all three methods.

Since, the API of `r BiocStyle::Biocpkg("proDA")` and the produced outputs are similar to those of prolfqua, we are planning to integrate the `r BiocStyle::Biocpkg("proDA")` package as an additional modeling option into `r BiocStyle::Githubpkg("fgcz/prolfqua")` (see Figure \@ref(fig:ContrastUML)), to make it easy for application developers to use both.

We focused our benchmark on comparing the statistical modeling methods using three different scores while we fixed the pre-processing steps. However, there are other equally or even more important parameters of a protein quantification pipeline [@frohlich2022benchmarking]. One of them is the normalization of the intensities within the samples to remove systematic differences [@pursiheimo2015optimization]. The method used to infer protein intensities from peptide intensities is an additional important factor [@Grossmann2010]. For instance, the original `r BiocStyle::Biocpkg("proDA")` publication uses MaxLFQ [@cox2014accurate] protein estimates. However, when using MaxLFQ intensities reported by MaxQuant, the $pAUC_{10}$ is significantly lower ($pAUC_{10}(\textrm{t-statistics}) = 66%$) compared with results obtained when protein abundances are estimated from peptide abundances using Tukey's median polish ($pAUC_{10}(\textrm{t-statistics}) = 75%$). Last but not least, the software [@Cox2008MaxQuant; @yu2020fast], identifying proteins and generating the quantification values, significantly contributes to the entire pipeline's performance, altering the number of identified proteins and the sensitivity and specificity of the differential expression analysis. In the supplement we show the benchmarking results for `r BiocStyle::Biocpkg("proDA")` and `r BiocStyle::Githubpkg("fgcz/prolfqua")` when the IonStar dataset is prepossessed with the MSFragger/IonQuant [@yu2020fast] quantification pipeline (v15). Although the number of proteins identified with two peptides is slightly lower than with MaxQuant (3929 vs. 4046), the $pAUC_{10}$ is significantly higher ($81%$ vs. $72%$).

# Conclusion

`r BiocStyle::Githubpkg("fgcz/prolfqua")` is a feature-rich, object-oriented R package to analyze quantitative mass spectrometric data with simple or complex experimental designs. While other R packages for differential expression analysis of proteins typically only implement one modeling approach, `r BiocStyle::Githubpkg("fgcz/prolfqua")` supports various types of models (see Figure 2 and Table 3). Furthermore, the contrast specification is explicit and consistent for all models and allows for testing interactions. Furthermore, the modular design of `r BiocStyle::Githubpkg("fgcz/prolfqua")` enables adding new features, e.g., generalized linear models to model the presence or absence information of a protein, or robust linear models, in the future. Furthermore, the developed framework is flexible enough to integrate other modeling methods, e.g., the probabilistic dropout model [@bioxrvproDA2020] or accurate variance estimation [@zhu2020deqms]. Hence, `r BiocStyle::Githubpkg("fgcz/prolfqua")` enables the implementation of applications where the user can select an alternative normalization method, protein abundance estimation method, or differential expression analysis algorithm. Furthermore, with minimal adaptations, this R package can analyze different quantitative proteomics data (e.g., labeling-based TMT-, PRM- DIA-data).
