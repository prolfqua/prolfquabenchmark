...
Supplementary Figure \@ref(fig:plotCPTACbarplot) shows the partial area under the curve for `r BiocStyle::Biocpkg("msqrob2")`, `r BiocStyle::Biocpkg("proDA")` and `r BiocStyle::Githubpkg("fgcz/prolfqua")`, when using the difference among the groups, the scaled p-values or the t-statistics to compute the ROC curves. For this dataset, the differences among groups show a higher performance than the t-statistic and scaled p-value. The *proDA* package performs slightly better then *prolfqua*  or *msqrob* although the differences are between the $pAUC_{10}$ for the scaled p-value score are not statistically significant. Table \@ref(tab:shwoROCtestREsultsCPTAC) shows the results of the Bootstrap test for two ROC curves. We observe that for this benchmark data, we can not reject the null hypothesis that there are no significant differences among the $pAUC_{10}$ (ROC based on scaled p-value) for the three packages. 

(ref:plotCPTACbarplot) Barplot showing the partial area under the ROC at 0.1 FDR. diff - differences among groups (logFC), scaled.p.value - $sign(diff) \cdot \textrm{p-value}$. 
...
Direct comparison with `msqrob2` is impossible since to fit the hurdle model, the peptide intensities are required, while here, we are starting the analysis from the `combined_proteins.tsv` file containing protein intensities. Starting from protein level intensities has the advantage that the effect size estimates obtained from the model agree with the input data, a feature frequently requested by the biologist.
