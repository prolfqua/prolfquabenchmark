...
Furthermore, we observe that the DEA benchmark results differ depending on if _MSstats.tsv_ or _combined_protein.tsv_ is used as input (see Figure \@ref(fig:compareMaxQuantFP) Left Panel, methods *proDA* and *prolfqua*). 
Supplementary Table \@ref(tab:shwoROCtestMQandFP) shows the p-values when we test if the $pAUC_{10}$, summarizing the DEA benchmark results, significantly differ between the *MaxQuant* and *FragPipe*. At significance level of $1%$ we will reject the null hypothesis that there is no difference between *FragPipe* and *MaxQuant*.
