...
Here we test if we should reject the null hypothesis that the area under the ROC curve at 0.1 FDR does not differ for 
msqrob2, proDA, and prolfqua when computed based on scaled p.values. To this task we use the function roc.test from the R package `r BiocStyle::CRANpkg("pROC")`.


```{r roctest, include = FALSE, eval = TRUE}
...
knitr::kable(tibble(pROC_test = names(tmp), p.value = round(tmp, digits = 2)), format = "latex", caption = cap)
...
We observe that for this benchmark data, there are no significant differences among the $AUC_{10}$ for the three packages. 
